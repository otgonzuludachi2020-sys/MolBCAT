...
class ChemBERTaDataset(Dataset):
    """
    Dataset for ChemBERTa models.
    """

    def __init__(self, smiles_list, label_list, max_len=128):

        self.encodings = CHEMBERTA_TOKENIZER(
            smiles_list,
            padding="max_length",
            truncation=True,
            max_length=max_len,
...
def make_chemberta_loaders(
    ...
    train_loader = DataLoader(
        ChemBERTaDataset(X_tr, y_tr),
        batch_size=batch_size,
        shuffle=True,
    )

    val_loader = DataLoader(
        ChemBERTaDataset(X_val, y_val),
        batch_size=batch_size,
        shuffle=False,
    )

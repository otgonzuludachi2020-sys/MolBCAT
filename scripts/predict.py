...
def _get_ckpt_path(weights_dir: str, model_name: str,
    ...
    if model_name == 'ChemBERTa':
        return os.path.join(weights_dir, 'ChemBERTa',
                            dataset, f'seed{seed}', 'ChemBERTa.pt')
...
def load_model(args, vocab: dict, device: str):
    ...
    if not os.path.exists(ckpt_path):
        ...
        print("  ├── ChemBERTa/{dataset}/seed{N}/ChemBERTa.pt")
...
def predict(smiles_list: list, model, vocab: dict,
    ...
    from src.dataset import build_graph_list, ChemBERTaDataset
    from torch.utils.data import DataLoader, TensorDataset
    from torch_geometric.loader import DataLoader as GeoLoader
    from src.models import GINModel, MolBCAT, ChemBERTaModel
    from src.trainer import infer_gru, infer_gin, infer_molbcat, infer_chemberta
    ...
    if model_name == 'MolBCAT':
        ...
        dummy_labels = [0.0] * len(valid_smiles)
        cb_ds     = ChemBERTaDataset(valid_smiles, dummy_labels, max_len=CHEMBERTA_MAX_LEN)
        cb_loader = DataLoader(cb_ds, batch_size=32, num_workers=0)
        p, _      = infer_chemberta(model, cb_loader, task, device)

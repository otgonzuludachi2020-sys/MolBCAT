...
from src.dataset import ChemBERTaDataset, make_chemberta_loaders
from src.models import GRUModel, GINModel, MolBCAT, ChemBERTaModel
from src.trainer import (
    train_gru, train_molbcat, train_chemberta,
    infer_gru, infer_gin, infer_molbcat, infer_chemberta
)
...
def run_phase1(dataset_cfg: dict, cfg: dict, vocab: dict, device: str) -> dict:
    ...
    all_keys = ['GRU_Random', 'GRU_Frozen', 'GRU_Finetune', 'GIN', 'MolBCAT', 'ChemBERTa']
    ...
    # ChemBERTa (full fine-tune, single lr — head uses lr*10)
    if 'ChemBERTa' not in best_hps:
        tl, vl = make_chemberta_loaders(X_tr, y_tr, X_val, y_val,
                                         cfg['training']['batch_size'])
        best_trial, best_val = None, float('inf')
        for trial in cfg['grids']['chemberta']:
            set_seed(cfg['split']['phase1_seed'])
            try:
                m = ChemBERTaModel().to(device)
                _, val_rmse, _, _ = train_chemberta(
                    m, tl, vl, None,
                    trial['lr'], trial['lr'] * 10,
                    cfg['training'].get('chemberta_epochs', cfg['training']['epochs']),
                    cfg['training']['early_stop'], 'reg', device)
                print(f"  ChemBERTa lr={trial['lr']} → RMSE={val_rmse:.4f}")
                if val_rmse < best_val:
                    best_val, best_trial = val_rmse, copy.deepcopy(trial)
            except Exception as e:
                print(f"  ChemBERTa trial failed: {e}")
        best_hps['ChemBERTa'] = best_trial
        save_json(hp_path, best_hps)
...
def run_phase2(dataset_cfg: dict, cfg: dict, vocab: dict,
    ...
    if remaining:
        ...
        for seed in remaining:
            ...
            # ChemBERTa
            try:
                cb_hp = best_hps.get('ChemBERTa', {'lr': 1e-5})
                tl_cb, vl_cb = make_chemberta_loaders(
                    X_tr, y_tr, X_val, y_val, cfg['training']['batch_size'])
                tl_test_cb   = DataLoader(ChemBERTaDataset(X_test, y_test),
                                          batch_size=cfg['training']['batch_size'],
                                          num_workers=_NW, pin_memory=_PIN)

                set_seed(seed)
                m_cb = ChemBERTaModel().to(device)
                lr = cb_hp['lr']
                m_cb, _, tr_curve_cb, vl_curve_cb = train_chemberta(
                    m_cb, tl_cb, vl_cb, None, lr, lr * 10,
                    cfg['training'].get('chemberta_epochs', cfg['training']['epochs']),
                    cfg['training']['early_stop'], 'reg', device)

                cb_preds, cb_labels = infer_chemberta(m_cb, tl_test_cb, 'reg', device)
                seed_results['ChemBERTa'] = calc_reg_metrics(cb_labels, cb_preds)
                torch.save(m_cb.state_dict(), os.path.join(ckpt_dir, 'ChemBERTa.pt'))

                curve_path_cb = os.path.join(out_dir, f'{name}_chemberta_curves.json')
                curves_cb = load_json(curve_path_cb)
                curves_cb[str(seed)] = {'train': tr_curve_cb, 'val': vl_curve_cb}
                save_json(curve_path_cb, curves_cb)

            except Exception as e:
                print(f"    ChemBERTa failed: {e}")
                seed_results['ChemBERTa'] = {k: float('nan') for k in ['RMSE', 'MAE', 'R2']}
            ...
            print(f"RMSE — Random:{seed_results['GRU_Random']['RMSE']:.4f}  "
                  ...
                  f"ChemBERTa:{seed_results['ChemBERTa']['RMSE']:.4f}")
...
def summarize(p2_data: dict, dataset_name: str, seeds: list) -> dict:
    ...
    for model_key in ['GRU_Random', 'GRU_Frozen', 'GRU_Finetune', 'GIN', 'MolBCAT', 'ChemBERTa']:
    ...
    cb = [p2_data[str(s)]['ChemBERTa']['RMSE']
          for s in seeds if str(s) in p2_data]
    if len(ca) >= 2 and len(cb) >= 2:
        from scipy.stats import ttest_rel
        result['p_MolBCAT_vs_ChemBERTa'] = float(ttest_rel(ca, cb).pvalue)
        result['d_MolBCAT_vs_ChemBERTa'] = cohens_d_paired(ca, cb)
    return result
...
def main(args=None):
    ...
    models  = ['GRU_Random', 'GRU_Frozen', 'GRU_Finetune', 'GIN', 'MolBCAT', 'ChemBERTa']
    metrics = ['RMSE', 'MAE', 'R2']

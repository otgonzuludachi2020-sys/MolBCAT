...
def run_phase1(dataset_cfg: dict, cfg: dict, vocab: dict, device: str) -> dict:
    ...
    all_keys = ['GRU_Random', 'GRU_Frozen', 'GRU_Finetune', 'GIN', 'ChemBERTa', 'MolBCAT']
        ...
        for seed in remaining:
            ...
            try:
                ...
                m_cb, _, tr_curve_cb, vl_curve_cb = train_chemberta(
                    m_cb, tl_cb, vl_cb, pos_weight, lr, lr * 10,
                    cfg['training'].get('chemberta_epochs', cfg['training']['epochs']),
                    cfg['training']['early_stop'], 'cls', device)

                cb_preds, cb_labels = infer_chemberta(m_cb, tl_test_cb, 'cls', device)
                seed_results['ChemBERTa'] = calc_cls_metrics(cb_labels, cb_preds)

                ckpt_chemberta = os.path.join(cfg['weights']['save_dir']['chemberta'],
                                               name, f'seed{seed}')
                os.makedirs(ckpt_chemberta, exist_ok=True)
                torch.save(m_cb.state_dict(), os.path.join(ckpt_chemberta, 'ChemBERTa.pt'))
                ...
                seed_results['ChemBERTa'] = {k: float('nan') for k in
                                              ['ROC_AUC', 'PR_AUC', 'Precision', 'Recall', 'F1']}
            ...
            print(f"    AUC — Random:{seed_results['GRU_Random']['ROC_AUC']:.4f}  "
                  ...
                  f"ChemBERTa:{seed_results['ChemBERTa']['ROC_AUC']:.4f}")
...
def summarize(p2_data: dict, dataset_name: str, seeds: list) -> dict:
    ...
    # Statistical comparison: MolBCAT vs ChemBERTa
    cb = [p2_data[str(s)]['ChemBERTa']['ROC_AUC']
          for s in seeds if str(s) in p2_data]
